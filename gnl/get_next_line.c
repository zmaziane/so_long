...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/21 23:25:08 by zmaziane          #+#    #+#             */
/*   Updated: 2022/02/24 21:27:01 by zmaziane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_lecture(int fd, char *buffers)
{
	...
	byte = 1;
	while (!ft_ft_strchr(buffers, '\n') && byte)
	{
		byte = read(fd, calu, BUFFER_SIZE);
		if (byte == -1)
			return (0);
		calu[byte] = '\0';
		buffers = ft_ft_strjoin(buffers, calu);
	}
	return (buffers);
}
